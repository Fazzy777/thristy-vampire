...
local FLY_SPEED, SEARCH_RADIUS = 150, 2000
...
local function initializeCharacterComponents()
    ...
    if LocalPlayer.Team == nil then
        Network:FireServer("TeamEvent", "RequestJoin", {Team = "Humans"})
        task.wait(5)
        if LocalPlayer.Team == nil then
            logToConsole("Human team full, joining Vampire team.")
            Network:FireServer("TeamEvent", "RequestJoin", {Team = "Vampires"})
        end
    end
    return true
end
            ...
            if tonumber(v.maxPlayers) > tonumber(v.playing) then
                ...
                if Possible == true then
                    ...
                    pcall(function()
                        writefile("NotSameServers.json", game:GetService('HttpService'):JSONEncode(AllIDs))
                        wait()
                        logToConsole("Hopping to server: " .. ID)
...
local function autoCollectLoop(delta)
    ...
    if not isFlyingToTarget and not isCollecting then
        ...
        if closestTarget then
            ...
            noSilverTimer = noSilverTimer + delta
            if noSilverTimer > 2 then
                logToConsole("No silver found for 2 seconds. Hopping server...")
                hopServer()
                noSilverTimer = 0
            end
        end
...
RunService.Heartbeat:Connect(function(delta) if isMasterEnabled then pcall(autoCollectLoop, delta) end end)
